...
function doLogin($username,$password)
{
        ...
        //Connect to DB
        $mydb = new mysqli('127.0.0.1','newuser','password','users');
...
function doRegister($userName,$userPass)
{
	...
	//Connect to DB
        $mydb = new mysqli('127.0.0.1','root','root','users');
        if ($mydb->errno != 0){
	...
	//Check is user already exists
        $fggquery = mysqli_query($mydb,"SELECT * FROM users WHERE user_name = '$userName' ");
        $count = mysqli_num_rows($query);
